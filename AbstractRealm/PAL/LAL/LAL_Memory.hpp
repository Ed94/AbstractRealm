...
namespace LAL
{
	...
	*/
	...
	template<typename Type>
	ptr<void> Format_ByFill(ptr<Type> _memoryAddress, Type& _fillValue, uDM _count)
	{
	...
	template<typename Type>
	ptr<void> Format_WithData(ptr<Type> _memoryAddress, ptr<const Type> _dataSource, uDM _count)
	{
	...
	#define Enforce_POD(_TYPE) \
	EnforceConstraint(Is_POD<_TYPE>(), "Not POD.");


	//#define Heap_Reserve(_AMOUNT)
}
